...
export interface UsageView {
  ...
  buckets: {
    ...
    name: string;
    plan?: string | null;
    windows: { remainingPercent: number; durationMinutes?: number | null; resetsAt?: string }[];
...
export class CodexUsage implements UsageBackend {
  private cached?: { at: number; executable: string; view: UsageView };
  private pending?: { epoch: number; executable: string; result: Promise<UsageView> };
  private epoch = 0;
  ...
  private view(data: RawUsage, cliVersion?: string): UsageView {
  ...
  read(refresh = false): Promise<UsageView> {
    const executable = selectedExecutable(this.executable),
      epoch = this.epoch;
