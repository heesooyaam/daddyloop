...
export async function backupArcadia(
  task: Pick<Task, 'id' | 'arcWorkspaces'>,
  sink: WorkspaceBackupSink,
  arc = new ArcBridge(sink.dataDir),
) {
  for (const [role, lease] of Object.entries(task.arcWorkspaces ?? {})) {
    const activation = lease.managed
      ? await arc.sessionMount(
          { id: task.id, groupId: lease.managed.sessionId },
          lease.managed.role,
        )
      : undefined;
    try {
      ...
      if (activation && !activation.wasMounted) await arc.parkSessionMount(lease);
    }
  }
}
