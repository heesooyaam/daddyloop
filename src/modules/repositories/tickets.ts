import { allRepositories } from './index.js';
...
export class TicketReader {
  ...
  async read(input: string): Promise<{ source: TicketSource; ref: TicketRef }> {
    const importer = this.repositories.forTicket(input);
    return importer.readTicket!(input, this);
  }
  async readBuiltin(input: string): Promise<{ source: TicketSource; ref: TicketRef }> {
