...
Фабрика может объявить `privatePaths()` для нативных секретов вне общей папки токенов; бекап исключит их, даже если модуль отключён. Версии CLI фиксируются для воспроизводимой поставки, а модели и уровни усилий приходят из живого каталога. Нельзя определять модуль по имени модели. См. [ревью](../agent-adapter-review/ru.md) и тесты жизненного цикла третьего движка, ошибок, восстановления и уведомлений.
