...
Follow-up in 0.4.3: native providers append hidden Reviewloop correlation comments to summaries. These are now excluded from Telegram presentation, with code examples and stored Markdown retained. Error-card wording also preserves uncertainty when an action may have completed before its confirmation was lost. Both cases have regression coverage; the suite totals 123 tests.
