...
При создании сессии или в её настройках открой **Стиль и навыки этой сессии**. Нажми **Добавить пресеты** и включи нужные наборы. Список загружается при открытии. **Создать пресет** открывает редактор здесь же; **Сохранить и подключить к сессии** сохраняет общий набор и добавляет его копию, не теряя черновик задачи. Пока открыт редактор пресета, запуск сессии недоступен. **К инструкциям сессии** возвращает без подключения. Если пресетов ещё нет, создай первый или напиши пожелания ниже. В разделе **Выбрать компоненты** можно отдельно включить или выключить промпт каждой роли и каждый навык. Пресеты применяются в показанном порядке выбора, а твой собственный промпт — после них. Одинаковый навык попадёт к роли один раз.
...
Импорт и экспорт JSON находятся в свёрнутом разделе **Перенести инструкции**.
