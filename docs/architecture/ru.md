...
**Папка.** Именованный воркспейс указывает на исходный репозиторий и область работы. `Workspaces` готовит отдельные копии для задачи, `ArcWorkspaces` делает это для Arcadia. Воркер меняет свою копию, ревьюер читает закреплённую версию кода. Для координации есть отдельная копия `DaddyWorkspace`. Исходную папку пользователя не переключают, незакоммиченные изменения не стирают.
