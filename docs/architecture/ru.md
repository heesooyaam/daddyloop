...
Инструкции сессии отделены от глобальных настроек моделей. При постановке запуска в очередь копируется текст нужной роли; новые настройки не меняют очередь и работающих агентов. `withInstructions` и `taskSession` собирают общий вход runtime для всех движков. Источник навыка служит указанием происхождения, а используется сохранённый текст; он входит в бекап. Импорт не запускает плагины и не выдаёт инструменты. Подробнее: [инструкции](../instructions/ru.md).
...
Выбор модулей хранится в конфиге. Выключенный модуль репозитория не принимает источник и не создаёт работу; выключенный движок не получает ход агента. Состав модулей собирается в точке запуска, очередь и daddy используют контракты. Точки расширения описаны в [модулях](../module-development/ru.md).
...
Пресеты инструкций — версионированные записи библиотеки `instructions.presets`. Выбор сессии содержит собственные снимки и ключи выключенных компонентов. `effectiveInstructions` объединяет активные части перед постановкой в очередь; адаптеры получают зафиксированный текст роли. Редактирование пресетов проверяет версию, а изменения библиотеки не обновляют копии сессий. Импорт папки в браузере собирает выбранные тексты целиком до изменения черновика.
