...
У координации свои границы: максимум **24 вызова инструментов за ход** и переход в `needs_input` после **8 автоматических ходов без изменения отпечатка ID/состояний/head задач**. Ввод пользователя или прогресс задач сбрасывает счётчик. При нехватке рабочих копий задание откладывается примерно на **30 секунд**.
