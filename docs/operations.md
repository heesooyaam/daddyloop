...
daddyloop is a single-user service on one Linux host. The API, SQLite, scheduler and both agent sessions belong to that host. CLI and browser clients can disconnect without stopping jobs. No tmux or laptop relay is required.

The installer places immutable versions in `~/.local/share/daddyloop/releases/<version>` and points `~/.local/bin/daddy` through `current`. Node, Codex CLI, GitHub CLI and UI assets are bundled. Git is supplied by the host or installed through apt. Reinstalling the same archive is idempotent; a different archive with the same version is rejected. Failed checksum verification does not switch the installed version. Installation changes files under the chosen prefix; setup additionally configures the OS service.

Configuration defaults to `~/.config/daddyloop/config.json` (`DADDYLOOP_CONFIG` overrides it). New installations store data in `~/.local/share/daddyloop/data`; an existing project's `.daddyloop/daddyloop.sqlite` is adopted on first setup. An explicit configured data directory takes precedence. Upgrades retain that path.

On unified-cgroup Linux, setup enables the user systemd manager and lingering. On older hybrid/v1 hosts it installs `daddyloop-<uid>.service` as a system unit with explicit `User` and `Group`. Agents still run as the ordinary user. The system service has its own memory cgroup. `daddy service status` reports the actual mode, PID, memory limit and logout persistence.
...
GitHub credentials: `GITHUB_TOKEN` / `GH_TOKEN`, `DADDYLOOP_GITHUB_TOKEN_FILE`, or `~/.tokens/github`. GitLab equivalents: `GITLAB_TOKEN` / `GLAB_TOKEN`, `DADDYLOOP_GITLAB_TOKEN_FILE`, `~/.tokens/gitlab`. Host-specific token files use `github-<host>` / `gitlab-<host>`. Configure a custom provider host with `DADDYLOOP_GITHUB_HOST` or `DADDYLOOP_GITLAB_HOST` when using a generic credential. A token is not sent to an arbitrary host from an attached URL.

`daddy auth github` uses gh's browser/device flow and saves a private credential file for the service. SSH authentication for Git is separate from API authentication. GitHub API permissions must allow reading code/checks and writing pull requests; automatic author push also requires repository write permission. GitLab needs a user API token with project access. Branch protection still applies.

Arc credentials are read from the existing Arc token environment or `~/.tokens/arcadia`. Public packages contain no corporate tools or credentials. `daddy arcadia setup` verifies the local helper, shared store and native identity. Author and reviewer each need a separate free clean configured mount. Leases are journaled before checkout, so an interrupted checkout is preserved and fails closed on recovery. Never remove that mount to clear an error without inspecting its changes.

Telegram setup needs a dedicated bot without an existing webhook. Save its token in `~/.tokens/daddyloop-telegram` or use the hidden setup prompt. `daddy telegram setup` activates polling and prints a one-use ten-minute private-chat pairing link. The binding checks both chat and user IDs; a connected forum routes the paired owner’s messages in session topics; unknown users receive no task data. A newly paired chat replaces the old binding.

Polling offsets and update receipts survive restarts and are reset when changing the bot identity. Duplicate callbacks cannot repeat a publication. A publish button expires after five minutes and binds the generation, head, review ID and exact draft snapshot. Editing the draft or advancing the revision invalidates it. `daddy telegram unpair` revokes the chat and outstanding buttons. Notifications are best effort: an ambiguous outgoing request is not blindly repeated, so a Telegram delivery failure can lose a notification. The database/UI remain authoritative. Telegram initialization errors do not prevent the website from starting.
...
daddy cache status
daddy cache prune
daddy cache prune --apply
daddy cache auto on
daddy restart
daddy cache arcadia-gc
daddy cache arcadia-gc --apply
```
...
| Another pending GitHub review exists                                      | Publish/discard it explicitly; daddyloop does not adopt or delete it.               |
