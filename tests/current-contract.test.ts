...
it('accepts only current configuration and profile names', () => {
  expect(configSchema.parse({}).version).toBe(2);
  expect(
...
it('initializes an empty database and rejects earlier schemas before changing user data', () => {
  ...
  try {
    ...
    expect(current.db.prepare('PRAGMA user_version').get()?.user_version).toBe(6);
    expect(
      current.db.prepare("SELECT name FROM sqlite_master WHERE name='workspaces'").get(),
    ).toBeDefined();
    current.close();
    for (const version of [0, 5, 7]) {
      ...
      before.close();
      expect(() => new Store(path)).toThrow('schema 6');
...
it('exposes the current CLI and rejects retired commands and model flags', () => {
  ...
  writeFileSync(config, JSON.stringify({ version: 2, dataDir: dir }));
  ...
  try {
    ...
    expect(profiles.stdout).toContain('--writer-model');
