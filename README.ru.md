...
**На ноутбуке**, в его локальном терминале:

```bash
ssh -N -o ExitOnForwardFailure=yes -L 4317:127.0.0.1:4317 user@server
```

Подставь свой обычный SSH-адрес. Оставь команду работающей и открой **[http://127.0.0.1:4317](http://127.0.0.1:4317) в браузере ноутбука**. Для входа выполни `daddy token` на сервере и вставь полученный токен в форму.
...
[Релиз 0.14.1](https://github.com/heesooyaam/daddyloop/releases/tag/v0.14.1) · [CI](https://github.com/heesooyaam/daddyloop/actions)
