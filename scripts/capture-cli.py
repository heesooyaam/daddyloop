...
def session(theme='dark', exercise=False):
    ...
    try:
        ...
        if exercise:
            ...
            start = command('/defaults'); wait('DEFAULTS FOR NEW TASKS', start)
            write('\r'); wait('Choose a model'); snapshot('cli-models'); write('\x1b')
            start = command('/notifications'); wait('TELEGRAM NOTIFICATIONS', start); snapshot('cli-notifications'); write('\x1b')
            start = command('/language ru'); wait('Подключено к сервису', start); snapshot('cli-ru')
            start = command('/models refresh'); wait('МОДЕЛИ ЭТОЙ ЗАДАЧИ', start); snapshot('cli-models-ru'); write('\x1b')
            start = command('/updates'); wait('Обновления', start); snapshot('cli-updates-ru'); write('\x1b')
            start = command('/language en'); wait('Connected to your service', start)
